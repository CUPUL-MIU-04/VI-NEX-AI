...
def main():
    ...
    logger.info("VI-NEX-AI Inference Configuration:\n %s", pformat(cfg.to_dict()))
    ...
    # 4. inference
    # ======================================================
    logger.info("Starting VI-NEX-AI inference...")
    
    # Estadísticas de inferencia
    total_samples = len(dataset) * num_sample
    logger.info("Total samples to generate: %s", total_samples)
    
    for epoch in range(num_sample):  # generate multiple samples with different seeds
        dataloader_iter = iter(dataloader)
        with tqdm(
            enumerate(dataloader_iter, start=0),
            desc=f"VI-NEX-AI Inference (Sample {epoch+1}/{num_sample})",
            ...
            for _, batch in pbar:
                ...
                if use_t2i2v:
                    ...
                    # Model offloading for memory optimization
                    if cfg.get("offload_model", False):
                        ...
                        logger.info(
                            "Offloaded video diffusion model to CPU, loaded image flux model to GPU: %.2f s",
                    ...
                    if cfg.get("offload_model", False):
                        ...
                        logger.info(
                            "Loaded video diffusion model to GPU, offloaded image flux model to CPU: %.2f s",
                            time.time() - model_move_start,
                        )
                ...
                logger.info("Generating video with VI-NEX-AI...")
                x = api_fn(
                    sampling_option,
                    cond_type,
                    seed=sampling_option.seed + epoch if sampling_option.seed else None,
    ...
    logger.info("VI-NEX-AI Inference finished successfully!")
    logger.info("Generated content saved to: %s", save_dir)
