...
def main():
    ...
    logger.info("VI-NEX-AI Training Configuration:\n %s", pformat(cfg.to_dict()))
    tb_writer = None
    if coordinator.is_master():
        tb_writer = create_tensorboard_writer(exp_dir)
        if cfg.get("wandb", False):
            wandb.init(
                project=cfg.get("wandb_project", "VI-NEX-AI"),
                name=exp_name,
                config=cfg.to_dict(),
                dir=exp_dir,
            )
    ...
    logger.info("VI-NEX-AI Training Setup:")
    logger.info("  - Number of GPUs: %s", num_gpus)
    logger.info("  - Number of groups: %s", num_groups)
    logger.info("  - Tensor Parallel: %s", tp_size)
    logger.info("  - Sequence Parallel: %s", sp_size)
    logger.info("  - Pipeline Parallel: %s", pp_size)

    # ======================================================
    # 2. build dataset and dataloader
    # ======================================================
    logger.info("Building VI-NEX-AI dataset...")
    # == build dataset ==
    ...
    # 3. build model
    # ======================================================
    logger.info("Building VI-NEX-AI models...")
    ...
    # =======================================================
    logger.info("Preparing VI-NEX-AI for distributed training...")
    # == boosting ==
    ...
    logger.info("VI-NEX-AI Training Schedule:")
    logger.info("  - Total epochs: %s", cfg_epochs)
    logger.info("  - Steps per epoch: %s", num_steps_per_epoch)
    logger.info("  - Total steps: %s", cfg_epochs * num_steps_per_epoch)
    ...
    if cfg.get("load", None) is not None:
        logger.info("Loading VI-NEX-AI checkpoint from %s", cfg.load)
    ...
    logger.info("Starting VI-NEX-AI training from epoch %s step %s", start_epoch, start_step)
    ...
    def prepare_inputs(batch):
        ...
        shift_alpha *= math.sqrt(x_0.shape[-3])
    ...
    def run_iter(inp, x_0, x_1):
        if is_pipeline_enabled(plugin_type, plugin_config):
            inp["target"] = (1 - sigma_min) * x_1 - x_0
    ...
    for epoch in range(start_epoch, cfg_epochs):
        ...
        logger.info("VI-NEX-AI Epoch %s/%s started", epoch + 1, cfg_epochs)

        with tqdm(
            enumerate(dataloader_iter, start=start_step),
            desc=f"VI-NEX-AI Epoch {epoch+1}/{cfg_epochs}",
            ...
            for _ in range(start_step, num_steps_per_epoch):
                ...
                with nsys.range("checkpoint"), timers["checkpoint"]:
                    if ckpt_every > 0 and actual_update_step % ckpt_every == 0:
                        ...
                        logger.info(
                            "VI-NEX-AI Checkpoint saved - Epoch: %s, Step: %s, Global Step: %s, Path: %s",
                            epoch,
                            step + 1,
                            actual_update_step,
                            save_dir,
                        ...
                        logger.info("Kept %s latest checkpoints.", cfg.get("keep_n_latest", -1))
    ...
    logger.info("VI-NEX-AI Training completed successfully!")
