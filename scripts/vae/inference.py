...
def main():
    ...
    logger.info("VI-NEX-AI VAE Inference Configuration:\n %s", pformat(cfg.to_dict()))
    ...
    logger.info("Starting VI-NEX-AI VAE inference...")
    logger.info("Saving original videos to: %s", save_dir_orig)
    logger.info("Saving reconstructed videos to: %s", save_dir_recn)
    ...
    with tqdm(
        ...
        for _, batch in pbar:
            ...
            running_sum += z.mean() * x.shape[0]
            running_var += (z - running_sum / num_samples).pow(2).mean() * x.shape[0]
            
            if num_samples % 100 == 0:
                current_mean = (running_sum / num_samples).item()
                current_std = (running_var / num_samples).sqrt().item()
                logger.info(
                    "VI-NEX-AI VAE feature stats (sample %s): mean=%.4f, std=%.4f",
                    num_samples, current_mean, current_std
                )
                pbar.set_postfix({"mean": f"{current_mean:.4f}", "std": f"{current_std:.4f}"})
    ...
    # Final statistics
    final_mean = (running_sum / num_samples).item()
    final_std = (running_var / num_samples).sqrt().item()
    logger.info("VI-NEX-AI VAE Inference completed!")
    logger.info("Final statistics - Samples: %s, Mean: %.4f, Std: %.4f", num_samples, final_mean, final_std)
