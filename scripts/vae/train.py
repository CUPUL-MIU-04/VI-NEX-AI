...
def main():
    ...
    logger.info("VI-NEX-AI VAE Training Configuration:\n %s", pformat(cfg.to_dict()))
    tb_writer = None
    if coordinator.is_master():
        tb_writer = create_tensorboard_writer(exp_dir)
        if cfg.get("wandb", False):
            wandb.init(
                project=cfg.get("wandb_project", "VI-NEX-AI-VAE"),
    ...
    # ======================================================
    logger.info("Building VI-NEX-AI VAE dataset...")
    # == build dataset ==
    ...
    # 3. build model
    # ======================================================
    logger.info("Building VI-NEX-AI VAE models...")
    ...
    # =======================================================
    logger.info("Preparing VI-NEX-AI VAE for distributed training...")
    # == boosting ==
    ...
    logger.info("Boosted VI-NEX-AI VAE model for distributed training")
    ...
    logger.info("VI-NEX-AI VAE Training Setup:")
    logger.info("  - Total epochs: %s", cfg_epochs)
    logger.info("  - Steps per epoch: %s", num_steps_per_epoch)
    logger.info("  - Mixed strategy: %s", mixed_strategy)
    logger.info("  - Mixed image ratio: %.2f (modulated: %.2f)", mixed_image_ratio, modulated_mixed_image_ratio)
    ...
    logger.info("VI-NEX-AI VAE Training for %s epochs with %s steps per epoch", cfg_epochs, num_steps_per_epoch)

    # == resume ==
    if cfg.get("load", None) is not None:
        logger.info("Loading VI-NEX-AI VAE checkpoint from %s", cfg.load)
        ...
        logger.info("Loaded VI-NEX-AI VAE checkpoint %s at epoch %s step %s", cfg.load, start_epoch, start_step)
    ...
    if cfg.get("freeze_layers", None) == "all":
        ...
        logger.info("All VI-NEX-AI VAE layers frozen")
    ...
    logger.info("Starting VI-NEX-AI VAE training loop...")
    
    for epoch in range(start_epoch, cfg_epochs):
        # == set dataloader to new epoch ==
        sampler.set_epoch(epoch)
        dataiter = iter(dataloader)
        logger.info("VI-NEX-AI VAE Epoch %s/%s started", epoch + 1, cfg_epochs)
        random.seed(1024 + dist.get_rank())  # load vid/img for each rank

        # == training loop in an epoch ==
        with tqdm(
            enumerate(dataiter, start=start_step),
            desc=f"VI-NEX-AI VAE Epoch {epoch+1}/{cfg_epochs}",
                ...
                for _ in range(start_step, num_steps_per_epoch):
                    ...
                    if (global_step + 1) % accumulation_steps == 0:
                        if coordinator.is_master() and actual_update_step % cfg.get("log_every", 1) == 0:
                            ...
                            pbar.set_postfix(
                                {
                                    **{k: f"{v:.4f}" for k, v in avg_loss.items()},
                        ...
                        if ckpt_every > 0 and actual_update_step % ckpt_every == 0:
                            ...
                            logger.info(
                                "VI-NEX-AI VAE Checkpoint saved - Epoch: %s, Step: %s, Global Step: %s, Path: %s",
                                epoch,
                                step + 1,
                                actual_update_step,
                                save_dir,
    ...
    logger.info("VI-NEX-AI VAE Training completed successfully!")
